...
def translate_file(
    ...
    code_writer.set_file_name(input_path)
    if bootstrap:
        code_writer.write_init()
    for _ in parser.valid_lines:
        code_writer.output_file.write("//"+parser.current_command+'\n')
        if parser.command_type() == "C_PUSH" or parser.command_type() == "C_POP":
            command = parser.command_type()
            segment_name = parser.arg1()
            index = parser.arg2()
            code_writer.write_push_pop(command, segment_name, index)
        ...
        if parser.command_type() == "C_FUNCTION":
            ...
            code_writer.current_function = function_name
            code_writer.write_function(function_name, args_num)
        if parser.command_type() == "C_CALL":
            function_name = parser.arg1()
            args_num = parser.arg2()
            code_writer.write_call(function_name, args_num)
